...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    await generateContractPdf({ adminUserId: admin.id, contractId: id });
    ...
    return NextResponse.redirect(new URL(`/admin/contracts/${id}/preview?error=${encodeURIComponent(code)}`, baseUrl), { status: 303 });
  }
}