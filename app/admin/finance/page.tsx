...
import { Banknote, CalendarClock, Clock3, Euro, Percent, ReceiptText, Repeat, Target, TrendingDown, TrendingUp, Users } from 'lucide-react';
...
import { calculateMonthlyEquivalentCents, formatRecurringCostFrequency, formatRecurringCostStatus, getNextRenewalLabel } from '@/lib/admin/finance-recurring-costs';
import { formatCurrencyCents, formatFinanceSource, formatFinanceTransactionStatus, formatFinanceTransactionType, formatSignedCurrencyCents } from '@/lib/finance/formatters';
...
export default async function AdminFinancePage({ searchParams }: FinancePageProps) {
  ...
  return (
    <div className="admin-page-grid finance-page">
      ...
      <AdminPanel title="Receita recorrente" subtitle="Acompanhe MRR, ARR, clientes ativos e mensalidade mÃ©dia da Norm8." action={<FinanceRecurringRevenueModal accounts={data.accounts} categories={data.categories} clientOptions={data.clientOptions} proposalOptionsByLeadId={data.proposalOptionsByLeadId} />}>
        <div className="admin-kpi-grid finance-kpi-grid">
          ...
          <AdminStatCard icon={<Users size={16} />} label="Clientes ativos" value={String(data.recurringRevenue.metrics.activeClients)} context="Clientes Ãºnicos com receita recorrente ativa." />
          <AdminStatCard icon={<Banknote size={16} />} label="Mensalidade mÃ©dia" value={formatCurrencyCents(data.recurringRevenue.metrics.averageMonthlyRevenueCents)} context="MRR dividido por clientes ativos." />
          <AdminStatCard icon={<ReceiptText size={16} />} label="Receita pontual" value={formatCurrencyCents(data.recurringRevenue.metrics.oneOffRevenueCents)} context="Entradas pontuais confirmadas ou pendentes no perÃ­odo." />
          <AdminStatCard icon={<Clock3 size={16} />} label="Receita recorrente mensal" value={formatCurrencyCents(data.recurringRevenue.metrics.recurringMonthlyRevenueCents)} context="MRR atual separado das implementaÃ§Ãµes pontuais." />
        </div>
      </AdminPanel>

      <AdminPanel title="Meta mensal" subtitle="Progresso simples para a meta interna de 10kâ‚¬/mÃªs.">
      ...
      <AdminPanel title="Custos recorrentes" subtitle="Acompanhe custos fixos, renovações e burn rate mensal da Norm8." action={<FinanceRecurringCostModal accounts={data.accounts} categories={data.categories} />}>
        <div className="admin-kpi-grid finance-kpi-grid">
          ...
          <div className="finance-recent-list">
            {data.recurringCosts.upcomingRenewals.map((cost) => (
              <article className="finance-recent-item" key={cost.id}>
                ...
                <div><strong>{cost.title}</strong><p>{cost.vendorName ?? cost.category?.name ?? 'Custo recorrente'} · {getNextRenewalLabel(cost.renewalDate)}</p></div>
          ...
          <AdminTable headers={['Custo', 'Fornecedor', 'Categoria', 'Frequência', 'Valor', 'Equiv. mensal', 'Renovação', 'Estado', 'Ações']}>
            {data.recurringCosts.costs.map((cost) => (
              <tr key={cost.id}>
                ...
                <td>{cost.renewalDate ? formatDatePt(cost.renewalDate) : 'Sem renovação'}</td>
      ...
      <AdminPanel title="Receitas recorrentes" subtitle={String(data.recurringRevenue.recurringRevenues.length) + ' mensalidades registadas'} action={<FinanceRecurringRevenueModal accounts={data.accounts} categories={data.categories} clientOptions={data.clientOptions} proposalOptionsByLeadId={data.proposalOptionsByLeadId} />}>
        {hasRecurringRevenues ? (
          <AdminTable headers={['Cliente', 'TÃ­tulo', 'Valor mensal', 'Estado', 'InÃ­cio', 'Fim', 'LigaÃ§Ãµes', 'AÃ§Ãµes']}>
            {data.recurringRevenue.recurringRevenues.map((recurringRevenue) => (
              <tr key={recurringRevenue.id}>
                <td><strong className="finance-table-title">{recurringRevenue.clientName}</strong>{recurringRevenue.billingDay ? <span className="finance-table-meta">CobranÃ§a dia {recurringRevenue.billingDay}</span> : null}</td>
      ...
      <AdminPanel title={'Transa\u00e7\u00f5es'} subtitle={String(data.transactions.length) + ' movimentos encontrados'}>
        {hasTransactions ? (
          <AdminTable headers={['Data', 'T\u00edtulo', 'Categoria', 'Tipo', 'Estado', 'Origem', 'Valor', 'A\u00e7\u00f5es']}>
            {data.transactions.map((transaction) => (
              <tr key={transaction.id}>
                <td>{formatDatePt(transaction.occurredAt)}</td>
