...
import { createContractDraft, updateCompanyLegalSettings, updateContractFromWizard, type ContractWizardInput } from './service';
...
export async function createContractDraftAction(formData: FormData): Promise<void> {
  ...
  const parsed = parseWizardForm(formData, admin.id);
...
function parseWizardForm(formData: FormData, adminUserId: string): { success: true; data: ContractWizardInput } | { success: false } {
